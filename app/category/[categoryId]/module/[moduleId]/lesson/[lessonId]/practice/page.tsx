...
import { trackLessonStart, trackRecordingStart, trackRecordingStop, trackLessonCompletion, trackError } from '@/lib/analytics/helpers'
...
export default function PracticePage() {
  ...
  const generateAudio = async (text: string) => {
    try {
      ...
      console.log('🔊 Generating audio for lesson explanation...')
      ...
      // Set both greeting and lesson audio
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-pink-50">
      ...
      <div className="bg-white/80 backdrop-blur-lg border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-6 py-3 flex justify-between items-center">
          <Link href={`/category/${categoryId}/modules`} className="text-slate-700 hover:text-purple-600 font-medium text-sm flex items-center gap-2">
      ...
      <div className="max-w-4xl mx-auto px-6 py-6">
        ...
        {step === 'intro' && (
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <h2 className="text-2xl font-bold text-slate-900 mb-4">{lessonData.title}</h2>
            
            <div className="bg-purple-50 border-2 border-purple-200 rounded-xl p-5 mb-6">
              <h3 className="font-bold text-purple-900 mb-3 flex items-center gap-2">
                <span className="text-xl">📚</span> Lesson Overview
              </h3>
              <p className="text-purple-800 leading-relaxed">{lessonData.explanation}</p>
            </div>

            {/* Audio Player - Compact & Clear */}
            {isAudioLoading && (
              <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl p-4 mb-6 text-center">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
              {!isRecording && !audioBlob && !isSubmitting && (
                <>
                  <h2 className="text-2xl font-bold text-slate-900 mb-8">Ready to Record</h2>
                  <button onClick={startRecording} className="w-28 h-28 bg-gradient-to-br from-purple-500 to-indigo-600 text-white rounded-full hover:shadow-2xl hover:scale-105 transition-all mx-auto flex items-center justify-center" type="button">
                    <Mic className="w-14 h-14" strokeWidth={2} />
                  </button>
                </>
              )}
              ...
              {isRecording && (
                <>
                  <h2 className="text-2xl font-bold text-red-600 mb-4">Recording...</h2>
                  <div className="text-4xl font-bold text-slate-900 mb-8">{formatTime(recordingTime)}</div>
                  <button onClick={stopRecording} className="w-28 h-28 bg-gradient-to-br from-red-500 to-pink-600 text-white rounded-full hover:shadow-2xl mx-auto flex items-center justify-center" type="button">
                    <Square className="w-10 h-10" strokeWidth={2} fill="white" />
                  </button>
                </>
              )}
              ...
              {audioBlob && !isSubmitting && (
                <>
                  ...
                  <h2 className="text-2xl font-bold text-green-600 mb-4">Recording Complete!</h2>
                  <p className="text-slate-600 mb-8">Duration: {formatTime(recordingTime)}</p>
                  <div className="flex gap-4 justify-center">
                    <button onClick={() => setAudioBlob(null)} className="px-6 py-3 bg-slate-200 text-slate-700 rounded-lg font-semibold hover:bg-slate-300" type="button">
                      Re-record
                    </button>
                    <button onClick={submitRecording} className="px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg font-semibold hover:shadow-xl" type="button">
                      Submit for Feedback →
                    </button>
                  </div>
                </>
              ...
              {isSubmitting && (
                <div className="py-8">
                  ...
                  <h2 className="text-2xl font-bold text-purple-600 mb-2">Analyzing Your Response</h2>
